    ...
    with tf.Session() as sess:
        for iter_cur in range(config.iter_num):
            ...
            for rl in range(config.rl_epoch):
                ...
                pos_list, l1,l1s = sess.run([pos, actor.log_softmax,actor.log_softmaxs], feed_dict={input: inputs, num_service: num})
                # print('pos_list: ',pos_list)
                # print('probs: ',probs)
                # abc=np.transpose(pos_list)
                # print("prob",abc)
                reward_list = []
                real_reward=[]
            ...
            wirter = tf.summary.FileWriter('./my_graph/1', sess.graph)
