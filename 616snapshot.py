...
class Pd3qnd(nn.Module):
    def __init__(self,L1,wd1,L2,wd2):
        ...
        self.demo_optimizer = torch.optim.Adam(self.eval_net.parameters(), lr=L2, weight_decay=wd2)
        self.loss_func = nn.MSELoss()
        self.ReplayMemory = Memory1(MEMORY_CAPACITY)
        self.demoReplayMemory = Memoryfd(MEMORY_CAPACITY)
        self.n_step = 3
        self.lamda1 = L1
        self.lamda2 = wd1
...
def generate_data(node_data):
    ...
    result = sorted(zip(random_combinations, rewards), key=lambda x: x[1], reverse=True)[:3]
...
def run_experiment(algorithm, algorithm_name, cut_point_add_to_plot=179):
    global EPSILON
    EPSILON = config.min_epsilon
    episodes = 720
    max_qos = -10000
...
#             print(position)
    ...
    plot_array.append(
        (y_matplot_smooth[:], np.arange(0, len(y_matplot_smooth[:])), last_50_episodes_mean, np.std(y_matplot)))
    print(f'{algorithm_name}-----------实验结束-----------, max_qos = {max_qos}')
    return plot_array,last_50_episodes_mean
def run_demo_experiment(algorithm, algorithm_name, cut_point_add_to_plot=179):
    global EPSILON
    EPSILON = config.min_epsilon
    episodes = 720
    max_qos = -10000
    ...
    # sampling_interval = 20
    window_size = 5  # 用于计算滑动平均的窗口大小
    epi_n = episodes / 18  # 每n个episode输出一次平均累积回报
    cumulative_reward_of_n_episodes = 0
    for i in tqdm(range(episodes)):
        state = 0
        position = []
...
#             print(position)
        ...
        if i != 0 and i % cut_point_add_to_plot == 0:
            ...
            if not os.path.exists(f'result/{current_time}/{algorithm_name}'):
                os.makedirs(f'result/{current_time}/{algorithm_name}')
            plt.savefig(f'result/{current_time}/{algorithm_name}/{algorithm_name}_training_episode_{i}.png')
    plot_array.append((y_matplot_smooth[:], np.arange(0, len(y_matplot_smooth[:])), last_50_episodes_mean, np.std(y_matplot)))
    print(f'{algorithm_name}-----------实验结束-----------, max_qos = {max_qos}')
    return plot_array,last_50_episodes_mean
...
def main():
    L1_list = [0,0.33,0.66,1]
    wd1_list = [0,0.33,0.66,1]
    L2_list = [5e-3]
    wd2_list = [1e-5,1e-6,1e-7]
    all_results = []
    for i in range(5):
        ...
        best_pd3qnd_results = []
        algorithms = {
            'PD3QND': None,  # We will update this later with the best instances
            'PD3QN11': Pd3qn11()
        }
        for algo_name, algo_instance in algorithms.items():
            if algo_name == 'PD3QN11':
                pd3qn11_y_matplot, pd3qn11_last_50_episodes_mean = run_experiment(algo_instance, algo_name)

        for L1 in L1_list:
            for wd1 in wd1_list:
                for L2 in L2_list:
                    for wd2 in wd2_list:
                        pd3qnd_instance = Pd3qnd(L1, wd1, L2, wd2)
                        y_matplot, last_50_episodes_mean = run_demo_experiment(pd3qnd_instance, 'PD3QND')
                        algo_name_with_params = f"PD3QND (L1={L1}, wd1={wd1}, L2={L2}, wd2={wd2})"
                        best_pd3qnd_results.append((algo_name_with_params, y_matplot, last_50_episodes_mean))

        # 只保留前5个最佳PD3QND实例
        pd3qn11_name = "PD3QN11"
        top_4_pd3qnd_results = sorted(best_pd3qnd_results, key=lambda x: x[2], reverse=True)[:4]
        top_5_results = sorted(top_4_pd3qnd_results + [(pd3qn11_name, pd3qn11_y_matplot, pd3qn11_last_50_episodes_mean)], key=lambda x: x[2], reverse=True)
        for name, _, mean_last_50 in top_5_results:
            all_results.append((name, mean_last_50))
        # 将最佳PD3QND实例的结果添加到algorithms_data中
        for name, plot_array, mean_last_50 in top_5_results:
            x_episode = list(range(1, len(plot_array) + 1))  # Update this line
            data = pd.DataFrame({'Episode': x_episode, 'Mean Cumulative Reward': plot_array})
            csv_dir = f'result/csv/{current_time}/{name}'
            if not os.path.exists(csv_dir):
                os.makedirs(csv_dir)
            data.to_csv(f'{csv_dir}/data_{sd}.csv', index=False)
        for result in top_5_results:
            algo_name, y_matplot, _ = result
            algorithms_data[algo_name] = y_matplot

        # 为每个随机种子绘制图形
        plot_comparison(algorithms_data, sd)
    all_results_sorted = sorted(all_results, key=lambda x: x[1], reverse=True)

    # Save all_results_sorted to a DataFrame
    all_results_df = pd.DataFrame(all_results_sorted, columns=['Algorithm', 'Last 50 Episodes Mean'])

    # Save the DataFrame to a CSV file
    csv_dir = f'result/csv/{current_time}'
    if not os.path.exists(csv_dir):
        os.makedirs(csv_dir)
    all_results_df.to_csv(f'{csv_dir}/all_results.csv', index=False)
